...
- [x] **Charla v2** (16 sep 2026): emojis por grupos y stickers, GIF por Tenor (secreto `TENOR_KEY`; sin llave se avisa y quedan los stickers), foto (se reduce en el navegador), video, nota de voz grabada, archivo, pegar y arrastrar, reacciones (❤️😂😮😢👍🔥, una por persona), "Visto ✓✓", separadores por día, rompehielos cuando solo hay saludos automáticos, envío optimista, sondeo cada 1.8 s solo con la pestaña visible.
...
- [ ] **GIF:** crear la llave de Tenor (gratis, Google Cloud → API Tenor) y guardarla con `wrangler secret put TENOR_KEY`.
- [ ] **Charla: tiempo real con Durable Objects** en vez de sondeo cuando haya cientos de charlas abiertas; avisos por correo de mensaje nuevo; reportar y bloquear; moderación.
